<!DOCTYPE html>
<html>
<head>
<meta name="viewport" content="width=device-width, initial-scale=1">
<link rel="stylesheet" href="jquery.mobile-1.4.5/jquery.mobile-1.4.5.min.css">
<script src="jquery.mobile-1.4.5/jquery-1.12.0.min.js"></script>
<script src="jquery.mobile-1.4.5/jquery.mobile-1.4.5.min.js"></script>

<!-- Functions Area -->
<script>

function fillmemno()
{
	document.getElementById('memno').value = document.getElementById('memtype').value + "-";
	
	
}
function calbalamt()
{
	document.getElementById("bamt").value = parseInt(document.getElementById("memamt").value) - parseInt(document.getElementById("pamt").value);
}


function login()
{
	
	var uname = document.getElementById('uname').value;
	var pass = document.getElementById('pass').value;
	
	  if (window.XMLHttpRequest)
	    {
            // code for IE7+, Firefox, Chrome, Opera, Safari
            xmlhttp = new XMLHttpRequest();
        } 
		else 
		{
            // code for IE6, IE5
            xmlhttp = new ActiveXObject("Microsoft.XMLHTTP");
        }
        xmlhttp.onreadystatechange = function()
		 {
			 
            if (xmlhttp.readyState == 4 && xmlhttp.status == 200)
			 { 
						
				var details = new Array();
			
				details = xmlhttp.responseText.split("|$|");
			
				var logname = details[0];			
			    var logtype = details[1];
				
				if("admin"==details[1])
				{
								
					document.getElementById('loginbtn').hidden = "true";
					document.getElementById('loginbtn').className = "";
					
					document.getElementById('logoutbtn').hidden = "false";
					document.getElementById('logoutbtn').className = "ui-btn";
					
					document.getElementById('mebtn').className = "ui-btn";
					
					
									
									
				}
				
					
					
			    
            }
        };
        xmlhttp.open("GET","php/login.php?uname="+uname+"&pass="+pass,true);
        xmlhttp.send();
	
}


function logout()
{
		
		 if (window.XMLHttpRequest)
		 {
            // code for IE7+, Firefox, Chrome, Opera, Safari
            xmlhttp = new XMLHttpRequest();
        } else
		{
            // code for IE6, IE5
            xmlhttp = new ActiveXObject("Microsoft.XMLHTTP");
        }
        xmlhttp.onreadystatechange = function()
		 {
			 
            if (xmlhttp.readyState == 4 && xmlhttp.status == 200)
			 { 
						
				
			
				details = xmlhttp.responseText;				
								
			    
					alert (details);
						
					document.getElementById('loginbtn').hidden = "false";
					document.getElementById('loginbtn').className = "ui-btn";
					
					document.getElementById('logoutbtn').hidden = "true";
					document.getElementById('logoutbtn').className = "";
					
					document.getElementById('mebtn').hidden = "true";
					document.getElementById('mebtn').className = "";
									
					
			    
            }
        };
        xmlhttp.open("GET","php/logout.php",true);
        xmlhttp.send();	
	
	
	
}


function searchmem()
{
	
	
	var idno = document.getElementById("memnos").value;
	
		 if (window.XMLHttpRequest)
		 {
            // code for IE7+, Firefox, Chrome, Opera, Safari
            xmlhttp = new XMLHttpRequest();
        } else
		{
            // code for IE6, IE5
            xmlhttp = new ActiveXObject("Microsoft.XMLHTTP");
        }
        xmlhttp.onreadystatechange = function()
		 {
			 
            if (xmlhttp.readyState == 4 && xmlhttp.status == 200)
			 { 
						
				
					var details = new Array();
					details = xmlhttp.responseText.split("|$|");
					
					var memimage = details[0];			
			   		var memname = details[1];
					var jdates = details[2];
					var rdates = details[3];
					var memamts = details[4];
					var pamts = details[5];
					var bamts = details[6];
					var rmarkss = details[7];
			
				
				document.getElementById('img001').src ="php/image.php?id="+idno;	
				document.getElementById('memnames').value= memname;	
				document.getElementById('jdates').value= jdates;	
				document.getElementById('rdates').value= rdates;
				
				document.getElementById('memamts').value= memamts;	
				document.getElementById('pamts').value= pamts;	
				document.getElementById('bamts').value= bamts;	
				document.getElementById('rmarkss').value= rmarkss;	
				document.getElementById('userfileu').value = memimage;
				
				
												
					
			    
            }
        };
        xmlhttp.open("GET","php/image.php?id="+idno,true);
        xmlhttp.send();	
	
	
}


function expirymem()
{
	
	
	var expirydatee = document.getElementById('expirydate').value;
	
		 if (window.XMLHttpRequest)
		 {
            // code for IE7+, Firefox, Chrome, Opera, Safari
            xmlhttp = new XMLHttpRequest();
        } else {
            // code for IE6, IE5
            xmlhttp = new ActiveXObject("Microsoft.XMLHTTP");
        }
        xmlhttp.onreadystatechange = function()
		 {
			 
            if (xmlhttp.readyState == 4 && xmlhttp.status == 200)
			 { 
						
				document.getElementById('expirydiv').innerHTML = xmlhttp.responseText;				
					
			    
            }
        };
        xmlhttp.open("GET","php/expirymemships.php?expirymemdate="+expirydatee,true);
        xmlhttp.send();	
	
	
}

function delete1()
{
		
	
	var memnodelete = document.getElementById("memnos").value;
	
		 if (window.XMLHttpRequest) {
            // code for IE7+, Firefox, Chrome, Opera, Safari
            xmlhttp = new XMLHttpRequest();
        } else {
            // code for IE6, IE5
            xmlhttp = new ActiveXObject("Microsoft.XMLHTTP");
        }
        xmlhttp.onreadystatechange = function() {
			 
            if (xmlhttp.readyState == 4 && xmlhttp.status == 200) { 
						
				/*document.getElementById('expirydiv').innerHTML = xmlhttp.responseText;	*/
				alert (xmlhttp.responseText);			
					
			    
            }
        };
        xmlhttp.open("GET","php/deletemember.php?memnodelete="+memnodelete,true);
        xmlhttp.send();	
	
}


function balpayfunc()
{
		
	
	
	
		 if (window.XMLHttpRequest) {
            // code for IE7+, Firefox, Chrome, Opera, Safari
            xmlhttp = new XMLHttpRequest();
        } else {
            // code for IE6, IE5
            xmlhttp = new ActiveXObject("Microsoft.XMLHTTP");
        }
        xmlhttp.onreadystatechange = function() {
			 
            if (xmlhttp.readyState == 4 && xmlhttp.status == 200) { 
			
							
				document.getElementById('balpaysdiv').innerHTML = xmlhttp.responseText;	
				
							
					
			    
            }
        };
        xmlhttp.open("GET","php/balpays.php",true);
        xmlhttp.send();	
	
}



function searchbyname()
{
		
	
	var searchbyname11 = document.getElementById("memnametype").value;
	
	//alert(searchbyname);
		 if (window.XMLHttpRequest) {
            // code for IE7+, Firefox, Chrome, Opera, Safari
            xmlhttp = new XMLHttpRequest();
        } else {
            // code for IE6, IE5
            xmlhttp = new ActiveXObject("Microsoft.XMLHTTP");
        }
        xmlhttp.onreadystatechange = function() {
			 
            if (xmlhttp.readyState == 4 && xmlhttp.status == 200) { 
			
							
				//document.getElementById('searchbynamediv').innerHTML = xmlhttp.responseText;	
					
				//document.getElementById('searchbynamediv').= memname;
				document.getElementById('searchbynamediv').innerHTML = xmlhttp.responseText;
				//document.getElementById('jdates').value= jdates;				
					
			    
            }
        };
        xmlhttp.open("GET","php/searchbyname.php?searchbyname1="+searchbyname11,true);
        xmlhttp.send();	
	
}



</script>





</head>
<body>


<!-- Login Page -->
<!-- Login Page -->
<!-- Login Page -->

<div data-role="page" id="login_pg">
  <div data-role="header">
    <h1 style="font-size:18px; margin-left:0;margin-right:0;padding-bottom:0;">LIFETIME FITNESS</h1>
    <p style="text-align:center; font-size:12px; margin-left:0;margin-right:0; padding-top:0;" >Udahamulla</p>
  </div>

   <div data-role="main" class="ui-content">
    <p>Please Login!!</p>
    
    <form>
      <label for="uname" class="ui-hidden-accessible">Username</label>
      <input type="text" name="uname" id="uname" placeholder="User name...">
      
      <label for="pass" class="ui-hidden-accessible">Password</label>
      <input type="password" name="pass" id="pass" placeholder="Password...">
      
      
       <a hidden="false" id="loginbtn" onClick="login()" class="ui-btn">Login</a>
       <a hidden="true" id="logoutbtn" onClick="logout()">Logout</a>
       <a hidden="false" id="mebtn" href="#home_pg" class="ui-btn">Home</a>
      
    </form>
            
  </div>

  <div data-role="footer">
    <h1></h1>
  </div>
</div> 

<!-- End Login Page -->


<!-- Home Page -->
<!-- Home Page -->
<!-- Home Page -->


<div data-role="page" id="home_pg">
  <div data-role="header">
  	<a href="index.php" class="ui-btn ui-icon-home ui-btn-icon-left">Logout</a>
    <h1 style="font-size:15px; padding-left:20px; padding-right:20px;">Home</h1>
  </div>

   <div data-role="main" class="ui-content">
	<div id="ss">Welcome <b><?php session_start(); echo $_SESSION["username"];?></b>!!!.. </div>
    
    <a href="#newmem_pg" class="ui-btn ui-btn-b">New Membership</a>
    <a href="#search_pg" class="ui-btn ui-btn-b">Search Member<br>(Edit,Delete)</a>
    <a href="#searchbyname_pg" class="ui-btn ui-btn-b">Search Member<br>(By Name)</a>
    <a href="#exmshps_pg" class="ui-btn ui-btn-b">Memberships to be Expired</a> 
    <a href="#balpays_pg" class="ui-btn ui-btn-b">Balance Payments</a>
    
    
  </div>

  <div data-role="footer">
    <h1 style="font-size:10px;">powered by ssnnetworking</h1>
  </div>
</div> 

<!-- End Home Page -->


<!-- search Member Page -->
<!-- search Member Page -->

<div data-role="page" id="search_pg">
  <div data-role="header">
  	<a href="#home_pg" class="ui-btn ui-icon-home ui-btn-icon-left">Home</a>   
    <h1 style="font-size:15px; margin:auto;"></h1>
    <a href="index.php" class="ui-btn ui-icon-power ui-btn-icon-right">Logout</a>
  </div>
  <h1 style="font-size:15px; margin:auto; text-align:center;">Search Membership</h1>
   
   <div data-role="main" class="ui-content">
      
       <form enctype="multipart/form-data" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>" data-ajax="false">
              <div class="ui-field-contain">
              
                <input type="text" name="memnos" id="memnos" placeholder="Membership no"/>
                <a class="ui-btn ui-btn-b" onClick="searchmem()">Search</a>
                
               <div align="center"> <img id="img001" style="max-height:100%;max-width:100%;" ></div>
                
                <label>Member Name :</label>
                 <input type="text" name="memnames" id="memnames" placeholder="Member Name"/><br>
                 
                 <label>Joined Date :</label>
                 <input type="date" id="jdates" name="jdates" placeholder="Joined Date" value=""/><br>
                 <label>Expiry Date :</label>
                 <input type="date" id="rdates" name="rdates" placeholder="Ending Date" value=""/><br>
                 
                 <label>Membership Amount :</label>
                 <input type="text" id="memamts" name="memamts" placeholder="Mem Amount" value=""/><br>
                 <label>Paid Amount :</label>
                 <input type="text" id="pamts" name="pamts" placeholder="Paid Amount" value=""/><br>
                 <label>Balance Amount :</label>
                 <input type="text" id="bamts" name="bamts" placeholder="Bal Amount" value=""/><br>
                 <label>Remarks :</label>
                 <input type="text" id="rmarkss" name="rmarkss" placeholder="Remarks" value=""/><br>
                 
                 <input name="userfileu" type="file" /><br>
                 
                 
                 <input type="submit" name="submit" value="Update"  /><br>
                 
                 <!-- <a class="ui-btn ui-btn-b" onClick="update()">Update</a><br> -->
                  <a class="ui-btn ui-btn-b" onClick="delete1()">Delete</a>              
                
                       
              </div>
      </form>      
  </div> 
  
  <?php
	    
	  if(isset($_POST['submit']))
	  {
		$memnou = $_POST['memnos'];
		 		  
		$memnameu = $_POST['memnames'];
					
		$jdateu = $_POST['jdates'];
		$datejju = strtotime($jdateu);
		$dateju = date('Y-m-d',$datejju);
			
		$edateu = $_POST['rdates'];
		$dateeeu = strtotime($edateu);
		$dateeu = date('Y-m-d',$dateeeu);
		
		$memamtu = $_POST['memamts'];
		$pamtu = $_POST['pamts'];
		$bamtu = $_POST['bamts'];
		$remarku = $_POST['rmarkss'];										
	
	
	
			
				
				require('php/connection.php');
				// echo $_FILES['userfile']['name'];
				
				
				
				
				if($_FILES['userfileu']['name'] == "")
			{	
				//$imagenameu = $_FILES['userfileu']['name'];
				//$imagedatau = addslashes(file_get_contents($_FILES['userfileu']['tmp_name']));
				
				
				$sql = "UPDATE memtb SET memname ='".$memnameu."',edate='".$dateeu."',jdate='".$dateju."',memamt='".$memamtu."',pamt='".$pamtu."',bamt='".$bamtu."',remark='".$remarku."' WHERE memno='".$memnou."' ";
						
						//$result = mysql_query($sql,$con);
						
						if (mysql_query($sql,$con))
						{
							echo "Record updated successfully";
						} else {
							echo "Error updating record: " . mysql_error($con);
						}
												
						
						//echo "Successfull...";
									
						mysql_close($con);
				
			}
			else
			{
				$imagenameu = $_FILES['userfileu']['name'];
				$imagedatau = addslashes(file_get_contents($_FILES['userfileu']['tmp_name']));
				
				
				$sql = "UPDATE memtb SET memname ='".$memnameu."',edate='".$dateeu."',rdate='".$dateju."',memamt='".$memamtu."',pamt='".$pamtu."',bamt='".$bamtu."',remark='".$remarku."',imgname='".$imagenameu."',image='".$imagedatau."' WHERE memno='".$memnou."' ";
						
						//$result = mysql_query($sql,$con);
						
						if (mysql_query($sql,$con))
						{
							echo "Record updated successfully";
						} else {
							echo "Error updating record: " . mysql_error($con);
						}
												
						
						//echo "Successfull...";
									
						mysql_close($con);
				
				
			}
								  
			
		  
		}
		else 
		{
		echo "update fail...";
		}
	  
	  
	  
	  
	  ?> 
  
  

  <div data-role="footer">
    <h1 style="font-size:10px;">powered by ssnnetworking</h1>
  </div>
</div> 


<!-- search Member Page END-->
<!-- search Member Page END-->
<!-- search Member Page END-->

<!-- search Member Page BY NAME-->
<!-- search Member Page BY NAME-->
<!-- search Member Page BY NAME-->


<div data-role="page" id="searchbyname_pg">
  <div data-role="header"> 
  <a href="#home_pg" class="ui-btn ui-icon-home ui-btn-icon-left">Home</a>   
    <h1 style="font-size:15px; margin:auto;"></h1>
    <a href="index.php" class="ui-btn ui-icon-power ui-btn-icon-right">Logout</a>
  </div>
  <h1 style="font-size:15px; margin:auto; text-align:center;">Search Member</h1>

   <div data-role="main" class="ui-content">
   
   <form>
   <input name="memnametype" id="memnametype" onChange="searchbyname()">
   <a class="ui-btn ui-btn-b" onClick="">Search Name</a>
     
   </form>
	    
        <div id="searchbynamediv"></div>
   
       <?php
	   require('php/connection.php');
	  
	   
		$sqla="SELECT memname FROM memtb";
		$qnames= mysql_query($sqla,$con);
			echo "<select name=\"pcid\">"; 
			echo "<option size =30 ></option>";
		
		while($row = mysql_fetch_array($qnames)) 
		{        
			echo "<option value='".$row['memname']."'>".$row['memname']."</option>"; 
		}
		mysql_close($con);
			echo "</select>";
			
		?>
      
    
    
    
  </div>

  <div data-role="footer">
    <h1 style="font-size:10px;">powered by ssnnetworking</h1>
  </div>
</div> 




<!-- search Member Page BY NAME END-->
<!-- search Member Page BY NAME END-->
<!-- search Member Page BY NAME END-->


<!--  expiry memship -->
<!-- expiry memship -->
<!--  expiry memship Page -->


<div data-role="page" id="exmshps_pg">
  <div data-role="header"> 
  <a href="#home_pg" class="ui-btn ui-icon-home ui-btn-icon-left">Home</a>   
    <h1 style="font-size:15px; margin:auto;"></h1>
    <a href="index.php" class="ui-btn ui-icon-power ui-btn-icon-right">Logout</a>
  </div>
  <h1 style="font-size:15px; margin:auto; text-align:center;">Expiry Membership</h1>

   <div data-role="main" class="ui-content">
   
   
	    
   <form>
       <label>Select Date</label>
      <input type="date" id="expirydate" />
      <a class="ui-btn ui-btn-b" onClick="expirymem()">Search</a> 
      
      <div id="expirydiv"></div>
      
    </form>
    
    
  </div>

  <div data-role="footer">
    <h1 style="font-size:10px;">powered by ssnnetworking</h1>
  </div>
</div> 

<!-- End expiry memship Page -->


<!--  Bal payments -->
<!-- Bal payments -->
<!--  Bal payments -->


<div data-role="page" id="balpays_pg">
  <div data-role="header"> 
  <a href="#home_pg" class="ui-btn ui-icon-home ui-btn-icon-left">Home</a>   
    <h1 style="font-size:15px; margin:auto;"></h1>
    <a href="index.php" class="ui-btn ui-icon-power ui-btn-icon-right">Logout</a>
  </div>
  <h1 style="font-size:15px; margin:auto; text-align:center;">Balance Payments</h1>

   <div data-role="main" class="ui-content">
   
    
      <a class="ui-btn ui-btn-b" onClick="balpayfunc()">Search BalPays</a> 
      
     
      <div id="balpaysdiv">
     
      
      
      
      </div>
      
   
    
    
  </div>

  <div data-role="footer">
    <h1 style="font-size:10px;">powered by ssnnetworking</h1>
  </div>
</div> 

<!-- End Bal Pay ments Page -->




<!-- New Member Page -->
<!-- New Member Page -->

<div data-role="page" id="newmem_pg">
  <div data-role="header">
  	<a href="#home_pg" class="ui-btn ui-icon-home ui-btn-icon-left">Home</a>   
    <h1 style="font-size:15px; margin:auto;"></h1>
    <a href="index.php" class="ui-btn ui-icon-power ui-btn-icon-right">Logout</a>
  </div>
  <h1 style="font-size:15px; margin:auto; text-align:center;">Add new Membership</h1>
   
   <div data-role="main" class="ui-content">
      <div class="ui-field-contain">
       <form enctype="multipart/form-data" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>" data-ajax="false">
              
                 
              
               <input type="text" name="name" id="name" placeholder="Full Name"/>
               
               	<select name="memtype" id="memtype" onChange="fillmemno()">
                      <optgroup label="Annual">
                      <option value="" selected>Select</option>
                        <option value="AIM">AIM</option>
                        <option value="ADM">ADM</option>
                        <option value="ACM">ACM</option>
                        <option value="AFM">AFM</option>
                      </optgroup>
                      <optgroup label="Monthly">
                        <option value="MM">MM</option>
                        <option value="TMM">TMM</option>
                        <option value="SMM">SMM</option>
                        <option value="TWMM">TWMM</option>                        
                      </optgroup>
        		</select>
                
                <input style="background-color:#0FF;" type="text" name="memno" id="memno" placeholder="Membership no"/><br>
                
                <hr>
                <label for="jdate">Joined Date</label>
                <input type="date" name="jdate" id="jdate"/><br>
                
                <label for="edate">Expiry Date</label>
                <input type="date" name="edate" id="edate"/>
                <br>
                <hr>
                
                <input type="text" name="memamt" id="memamt" placeholder="Amount"/>
                
                <input type="text" name="pamt" id="pamt" placeholder="Paid Amount" onChange="calbalamt()" />
                
                <input type="text" name="bamt" id="bamt" placeholder="Balance Amount" /><br>
                
                <textarea name="remark" id="remark" placeholder="Remarks"></textarea>
                
                <hr>
                
                     <input name="userfile" type="file" /><br>
                               
               <input type="submit" name="submit" value="Insert" />         
                       
             
      </form> 
       </div>
      
      <?php
	  
	  require('php/connection.php');
	  
	  if(isset($_POST['submit']))
	  {
		 		  
	$memname = $_POST['name'];
	$memtype = $_POST['memtype'];
	$memno = $_POST['memno'];	
			
	$jdate = $_POST['jdate'];
	$datejj = strtotime($jdate);
	$datej = date('Y-m-d',$datejj);
		
	$edate = $_POST['edate'];
	$dateee = strtotime($edate);
	$datee = date('Y-m-d',$dateee);
	
	$memamt = $_POST['memamt'];
	$pamt = $_POST['pamt'];
	$bamt = $_POST['bamt'];
	$remark = $_POST['remark'];
	
	
	
if(!isset($_FILES['userfile']))
{
    echo '<p>Please select a file</p>';
}
else
{
	
	// echo $_FILES['userfile']['name'];
	

	$imagename = $_FILES['userfile']['name'];
	$imagedata = addslashes(file_get_contents($_FILES['userfile']['tmp_name']));
	
	

			$sql = "INSERT INTO memtb(memname,memtype,memno,jdate,edate,rdate,memamt,pamt,bamt,remark,imgname,image) VALUES ('".$memname."','".$memtype."','".$memno."','".$datej."','".$datee."','".$datej."','".$memamt."','".$pamt."','".$bamt."','".$remark."','".$imagename."','".$imagedata."')";
			
			$result = mysql_query($sql,$con);
			
			echo $imagename;
						
			mysql_close($con);
		  
		  
		  }
		  
		}
	  
	  
	  
	  
	  ?>  
  
       
    
  </div>

  <div data-role="footer">
    <h1 style="font-size:10px;">powered by ssnnetworking</h1>
  </div>
</div> 


<!-- End New Member Page -->






</body>
</html>
